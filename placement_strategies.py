...
class LatencyAwarePlacementStrategy(PlacementStrategy):
    """
    Alternative: Place replicas on nodes with lowest average latency to consumers
    Greedy approach that's faster than p-center for large p
    """
    ...
    def get_name(self) -> str:
        return f"latency_aware_{self.num_replicas}_replicas"
    ...
    def place(
        ...
        # Score each node by average latency to all consumers
        node_scores = []
        for node_id in available_nodes:
            avg_latency = sum(
                context.latency_cache.get_latency(consumer_node, node_id)
                for consumer_node in consumer_nodes
            ) / len(consumer_nodes)
            node_scores.append((node_id, avg_latency))

        # Select top p nodes with lowest average latency
        node_scores.sort(key=lambda x: x[1])
        return [node_id for node_id, _ in node_scores[:self.num_replicas]]


class HierarchicalPlacementStrategy(PlacementStrategy):
    """
    Alternative: Place replicas at different hierarchy levels (cloud, fog, edge)
    Ensures geographic diversity and fault tolerance
    """

    def get_name(self) -> str:
        return "hierarchical_placement"
    ...
    def place(
        ...
        # Categorize available nodes by type
        nodes_by_type = {"cloud": [], "fog": [], "edge": []}

        for node_id in context.get_available_storage_nodes(data_size_gb):
            node_type = context.topology.nodes[node_id].get("type")
            if node_type in nodes_by_type:
                nodes_by_type[node_type].append(node_id)

        selected_nodes = []

        # Try to place one replica at each level
        for level in ["cloud", "fog", "edge"]:
            if nodes_by_type[level]:
                selected_nodes.append(context.rng.choice(nodes_by_type[level]))
                if len(selected_nodes) >= 3:
                    break

        # If we don't have 3 replicas yet, add more from available nodes
        if len(selected_nodes) < 3:
            all_available = context.get_available_storage_nodes(data_size_gb)
            remaining = [n for n in all_available if n not in selected_nodes]
            needed = min(3 - len(selected_nodes), len(remaining))
            selected_nodes.extend(context.rng.sample(remaining, needed))
